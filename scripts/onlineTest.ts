...
async function main() {
  ...
  const CreatorDAOCommission = await ethers.getContractFactory(
  ...
  const gmFans = CreatorDAOCommission.attach(
    "0x28661723731bd8703044b2bf18ea790ea0f5536c"
  );
  //   console.log("gmfans", instance.address);

  // expect(await gmFans.newShopIndex()).to.be.equal(1);
  await expect(await gmFans.addShop(1, 50, deployer.address))
    .to.emit(gmFans, "NewShop")
    .withArgs("1", "1", "50", deployer.address);
