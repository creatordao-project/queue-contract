...
contract CreatorDAOCommission is Initializable{
    ...
    function initialize(address payable _admin, address payable _recipientDao) public initializer {
        admin = _admin;
    ...
    callNotStarted
    {
        ...
        (bool success, ) = selectedCommission.recipient.call{value: selectedCommission.bid}("");
    ...
    callNotStarted
    {
        ...
        for (uint256 i = 0; i < _commissionIndexes.length; i++){
            ...
            require(selectedCommission.status == CommissionStatus.queued, "commission not in the queue");

            selectedCommission.status = CommissionStatus.accepted; // first, we change the status of the commission to accepted

            uint256 taxAmount = selectedCommission.bid * shops[selectedCommission.shopId].tax / 1000;

            uint256 payAmount = selectedCommission.bid - taxAmount;

            totalTaxAmount = totalTaxAmount + taxAmount;

            (bool success, ) = shops[selectedCommission.shopId].owner.call{value: payAmount}(""); // next we accept the payment for the commission
            require(success, "Transfer failed.");
